...
int GetFig_Fun(In3str1v * paraFA04, Para_18 * para_18 )
{
	...
	{
		command=command+"\t-InGWAS\t"+(paraFA04->InGWAS);
	}

	if  (!(paraFA04->InGFF).empty())
	{
		command=command+"\t-InGFF\t"+(paraFA04->InGFF);
	}

	if  (!(paraFA04->MerMinSNPNum).empty())
	{
		command=command+"\t-MerMinSNPNum\t"+(paraFA04->MerMinSNPNum);
	}

	if  ((paraFA04->OutPng))
	{
		command=command+"\t-OutPng\t" ;
	}

	if  ((paraFA04->OutPdf))
	{
		command=command+"\t-OutPdf\t" ;
	}
