#ifndef PairWiseDRaw_H_
#define PairWiseDRaw_H_

using namespace std;


int Region_PairWise_DRaw(In3str1v * paraFA04, Para_18 * para_18 , map <string,map <llong, vector <BaseType> > >  & SNPList, int & Flag_for_pro)
{

	string StatDRaw  =(paraFA04->InStr2)+".TriangleV.gz";
	string StatSite=(paraFA04->InStr2)+".site.gz";
	string StatBlocks=(paraFA04->InStr2)+".blocks.gz";
    ogzstream OUTAA((StatDRaw).c_str());
    if(!OUTAA.good())
	{
		cerr << "open OUT File error: "<<StatDRaw<<endl;
		return 0;
	}
	...
	OUTAA<<"#Region PairWise D raw"<<endl;
	vector<string> inf;
	...
	for(  ; key2!=NewSNPList.end(); key2++ )
	{
		...
		if ( key2_se!=NewSNPList.end() )
		{
			...
			cal_RR_DRaw(key2->second , key2_se->second ,CalResult, Var);
	...
	if  ( (paraFA04->Method)==1 )
	{
		...
		{
			cout <<"#Warning:\tfind blocks Method 1 cannot accept genotype format, you can try other Method 2/3 ...\n";
			return 1;
		}

		stringstream   sstrmMAF , sstrmMiss,sstrmHWE;
		sstrmMAF  <<  (para_18->MAF)  ;
		sstrmMiss <<  (para_18->Miss) ;
		sstrmHWE <<  (para_18->HWE) ;
		string MissStr=sstrmMiss.str();
		if (is_digits(chrName))
		{
			int chrN=atoi(chrName.c_str());
			if (chrN>24)
			{
				MissStr = MissStr+"  --chr-set  " + chrName;
			}
		}
		string Plink=(paraFA04->BinDir)+"/plink";
		if ( access(Plink.c_str(), 0) == 0 )
		{
			Plink=Plink+" --vcf "+(paraFA04->InStr1)+" --snps-only   --geno  " + MissStr +"  -maf " + sstrmMAF.str()  + " -hwe   " + sstrmHWE.str() +  " --blocks   no-pheno-req  --allow-extra-chr   --out   "+ (paraFA04->InStr2)+".plink > /dev/null  ";
