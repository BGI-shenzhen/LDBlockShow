...
int LDdecay_help01(int argc, char **argv , In3str1v * paraFA04, Para_18 * para_18)
{
	...
	for(int i = 1; i < argc ; i++)
	{
		...
		else if (flag  == "NumGradien")
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
			i++;
			(paraFA04->InGWAS)=(paraFA04->InGWAS)+"\t-NumGradien\t"+argv[i];
		}
		else if (flag  == "InGFF")
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
		...
		else if (flag  == "NoHeatMapPlot")
		{
			(paraFA04->InGWAS)=(paraFA04->InGWAS)+"\t-NoHeatMapPlot";
		}
		...
		else if (flag == "EnableOthVar")
		{
			para_18->IFIndel=false;
		}
		else if (flag == "HWE")
		{
			if(i + 1== argc) {LogLackArg(flag);return 0;}
